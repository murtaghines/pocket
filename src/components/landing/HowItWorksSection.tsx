    ...
    <section className="bg-white">
      {panels.map((p, i) => (
        ...
        >
          ...
          <div className="absolute bottom-12 left-16 w-6 h-6 rounded-full bg-[#0F4264]/20" />
          <div className="absolute top-1/2 right-1/3 w-3 h-3 rounded-full bg-[#3391D0]/40" />
          <img
            src={pocketIcon}
            alt=""
          ...
          <div className="container px-6 py-20 lg:py-32 relative z-10">
            <div className="flex flex-col lg:flex-row items-start lg:items-center gap-8 lg:gap-12">
              ...
              <div className="shrink-0 w-32 h-32 lg:w-44 lg:h-44 rounded-3xl bg-[#0F4264] flex items-center justify-center shadow-xl rotate-[-6deg]">
                <p.Icon className="w-14 h-14 lg:w-20 lg:h-20 text-white" strokeWidth={1.5} />
              </div>

              <div className="flex-1">
                <div className="text-xs font-bold tracking-[0.25em] text-[#0F4264]/70 mb-3">
                  STEP {String(i + 1).padStart(2, "0")}
                </div>
                <h3
                  className="font-black text-[#0F4264] uppercase leading-[0.92] tracking-tight"
                  style={{ fontSize: "clamp(2.25rem, 6vw, 5rem)" }}
                >
                  {p.title}
                </h3>
                <p className="mt-4 text-lg lg:text-xl text-[#0F4264]/80 max-w-xl">{p.sub}</p>
              </div>
            </div>
          </div>
        </div>
