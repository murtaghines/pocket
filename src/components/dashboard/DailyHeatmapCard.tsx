...
export function DailyHeatmapCard({ transactions, monthKey, convert }: DailyHeatmapCardProps) {
  ...
  const { cells, maxValue, minNonZero, daysInMonth, leadingBlanks } = useMemo(() => {
    if (!monthKey) {
      return { cells: [] as Array<{ day: number; value: number }>, maxValue: 0, minNonZero: 0, daysInMonth: 0, leadingBlanks: 0 };
    }
    ...
    return { cells, maxValue, minNonZero, daysInMonth: days, leadingBlanks: firstWeekday };
  ...
  return (
    <div
      className="bg-card rounded-[18px] p-[18px_20px_16px] h-full"
