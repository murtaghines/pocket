...
const PILL = "flex items-center gap-[6px] text-[13px] font-medium text-foreground/80 bg-card rounded-[10px] px-[13px] py-[8px] cursor-pointer whitespace-nowrap select-none hover:bg-muted/40 transition-colors shadow-[0_1px_2px_rgba(16,24,40,.05)]";
...
function YearPill() {
  ...
  return (
    <span className={NAV_PILL}>
      ...
      <span className="min-w-[40px] text-center text-[13px] font-medium text-foreground/80">{label}</span>
    ...
    <div className="hidden md:block py-[18px] sticky top-0 z-30 bg-background">
      <div className="flex items-center justify-between">
            ...
            <p className="text-[13px] text-muted-foreground mt-[2px]">
              {t("greeting.openingBalanceSubtitle", {
                ...
                i < arr.length - 1 ? (
                  <span key={i}>
                    {part}
                    <span className="font-medium text-foreground/80 tabular-nums">{formatCurrency(openingBalance)}</span>
                  </span>
                ) : (
        ...
        <div className="flex items-center gap-2 mt-[2px]">
          <EmptyStateBanner />
          <span className={PILL}>
            <SlidersHorizontal className="w-[14px] h-[14px] text-muted-foreground" strokeWidth={2} />
            {t("greeting.filter")}
          </span>
          {tab === "month" && <MonthPill />}
          {tab === "week" && <WeekPill />}
