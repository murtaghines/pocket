-- Fix: sent_to_invest was double-counting Revolut pocket transfers.
-- Revolut exports both sides of internal savings-pocket moves as separate
-- rows in the same account (e.g. "To Savings" -1000 and +1000).
-- SUM(ABS(amount)) counted both, inflating the total ~2x.
--
-- Fix: use -SUM(amount) (net outflow) instead of SUM(ABS(amount)).
-- Paired entries cancel out; only genuine one-sided transfers remain.
-- Floor at 0 so months with net withdrawals from savings show 0.

--------------------------------------------------------------------------------
-- 1. Recreate mv_daily_totals with fixed sent_to_invest
--------------------------------------------------------------------------------
...
-- get_monthly_series (reads from mv_daily_totals)
...
-- get_period_series (reads from mv_daily_totals)
...
-- get_dashboard_aggregates (reads directly from transactions — fix inline)
...
-- get_dashboard_summary (reads directly from transactions — fix inline)
...
AS $$
  SELECT
    ...
    COALESCE(SUM(ABS(amount)) FILTER (WHERE movement = 'INCOME'), 0)
      ...
      AS balance,
    GREATEST(
      -COALESCE(SUM(amount) FILTER (WHERE movement = 'TRANSFER' AND category = 'to_investment'), 0),
      0
    ) AS sent_to_invest,
    COUNT(*)
      AS tx_count
